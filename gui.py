...
# Function to load the Facial Expression Model from JSON and weights
def FacialExpressionModel(json_file, weights_file):
    with open(json_file, "r") as file:
        loaded_model_json = file.read()
        model = model_from_json(loaded_model_json)
...
# Initialize label and sign_image
label1 = Label(top, background='#CDCDCD', font=('arial', 15, 'bold'))
label1.pack(side='bottom', expand=True)
...
# Specify the correct path to haarcascade_frontalface_default.xml
cascade_path = 'haarcascade_frontalface_default.xml'

# Initialize cascade classifier for face detection
facec = cv2.CascadeClassifier(cascade_path)
if facec.empty():
    raise IOError(f"Unable to load the cascade classifier xml file at {cascade_path}")
...
# Function to detect emotion from an uploaded image
def Detect(file_path):
    try:
...
# Function to display the "Detect Emotion" button
def show_Detect_button(file_path):
    detect_b = Button(top, text="Detect Emotion", command=lambda: Detect(file_path), padx=10, pady=5)
    detect_b.configure(background="#364156", foreground='white', font=('arial', 10, 'bold'))
    detect_b.place(relx=0.79, rely=0.46)
